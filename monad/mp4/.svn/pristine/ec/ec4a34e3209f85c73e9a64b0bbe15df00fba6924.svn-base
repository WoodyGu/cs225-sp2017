...
T Queue<T>::dequeue()
{
     ...
     */
     //if the outstack is not empty, we can just return whatever is on the top
     //if the outstack is empty, push everything in instack to outstack
   /*while(!(inStack.isEmpty())){
    	outStack.push(inStack.pop());
    }
    T retval = outStack.pop();
...
T Queue<T>::peek()
{
     ...
     * @todo Your code here! You will need to replace the following line.
     */
     //same idea, always be the same element
     /*while(!(inStack.isEmpty())){
    	outStack.push(inStack.pop());
    }
    T retval = outStack.pop();
    outStack.push(retval);
    while(!(outStack.isEmpty())){
    	inStack.push(outStack.pop());
    }
    return retval;
