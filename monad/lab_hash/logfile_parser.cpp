 ...
 */
LogfileParser::LogfileParser(const string& fname) : whenVisitedTable(256)//<string,time_t>put time in
{
    SCHashTable<string, bool> pageVisitedTable(256);//put only url in, the goal is to see if the URL is visited
    ...
    while (infile.good()) {
         ...
         */
         ...
         if(!pageVisitedTable.keyExists(ll.url)){
           ...
           this->uniqueURLs.push_back(ll.url);
         }
         //does the key already exist in the table
         //if does, do nothing, if not insert into table with true
         //push the url into the vector
    }
    infile.close();
}
