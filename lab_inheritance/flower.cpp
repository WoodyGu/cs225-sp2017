...
Flower::Flower(const Vector2& center)//slicing occurs, those object are not allow to access their individual constructor, they can only use shape's constructor
{//can be fixed by letting Shape stem >> Shape* stem, and use copy constructor to make a new object on heap and let the shape pointer point to it.
