...
class KDTree
{
  ...
  private:
     ...
     */
     ...
     int partition(vector<Point<Dim>>& pointArray,int lo,int hi,int pivotIndex,int currDim);
     Point<Dim> quickSelect(vector<Point<Dim>>& pointArray,int lo,int hi,int pivotIndex, int currDim);
     void swap(vector<Point<Dim>>& pointArray,int first,int second);
     void buildTreeHelper(vector<Point<Dim>>& newPoints, int lo, int hi,int currDim);
     Point<Dim> findNearestHelper(const vector<Point<Dim>>& pointArray,const Point<Dim>& query,int lo,int hi,int currDim) const;
};
