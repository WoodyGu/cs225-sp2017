...
V BTree<K, V>::find(const BTreeNode* subroot, const K& key) const
{
     ...
     */
     ...
     if(subroot->is_leaf){//if it is at leaf, we did not find it
       return V();//not found!
     }
     else{
       BTreeNode* b = subroot->children[first_larger_idx];//(i th subtree)
       return find(b,key);
     }
     //return V();
}
...
void BTree<K, V>::split_child(BTreeNode* parent, size_t child_idx)
{
    ...
    parent->children.insert(child_itr+1,new_right);
    */
    //condition: new_left and new_right's children vector
    //child's children's vector is not empty

    parent->children.insert(child_itr,new_right);
    parent->elements.insert(elem_itr,*mid_elem_itr);

    parent->children[child_idx+1]->elements.assign(mid_elem_itr+1,child->elements.end());
    parent->children[child_idx+1]->children.assign(mid_child_itr,child->children.end());
...
void BTree<K, V>::insert(BTreeNode* subroot, const DataPair& pair)
{
    ...
    if(subroot->is_leaf){//if the subroot is a leaf node
      //if the key doesn't exist subroot
      subroot->elements.insert(subroot->elements.begin()+ first_larger_idx, pair);

    }
    else{//recursive call to insert the next children
      insert(subroot->children[first_larger_idx],pair);
      if(subroot->children[first_larger_idx]->elements.size()>= order){//see if the size is greater than the order
