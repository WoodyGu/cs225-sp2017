...
size_t insertion_idxHelper(const std::vector<T>& elements, const C& val,int lo, int hi){
   int mid = (lo+hi)/2;
   if(lo>=hi){
     if(elements[mid]<val){
       return lo+1;
   ...
   if(elements[mid] == val){
     ...
     return insertion_idxHelper(elements,val,mid+1,hi);
   }
   else{
     return insertion_idxHelper(elements,val,lo,mid-1);
   }
}
...
size_t insertion_idx(const std::vector<T>& elements, const C& val)
{
    ...
    return insertion_idxHelper(elements,val,0,elements.size()-1);
}
