...
RGBAPixel gradientColorPicker::operator()(int x, int y)
{
     ...
     */
    int distance = abs(centX-x) + abs(centY-y);
    if(distance>gradientRadius){
      return color2;
    }
    color.red = color1.red - ((distance*color1.red)/gradientRadius) + ((distance*color2.red)/gradientRadius);
    color.green = color1.green - ((distance*color1.green)/gradientRadius) + ((distance*color2.green)/gradientRadius);
    color.blue = color1.blue - ((distance*color1.blue)/gradientRadius) + ((distance*color2.blue)/gradientRadius);
    return color;
}
