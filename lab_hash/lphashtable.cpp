...
void LPHashTable<K, V>::insert(K const& key, V const& value)
{
     ...
     */
     ...
     while(should_probe[index] == true){
       index = (index+1)%size;//should add a mod size so after we reach the end of the table, it will loop back to the beginning of the table
     }
...
template <class K, class V>
void LPHashTable<K, V>::remove(K const& key)//why don't set the should probe?
{
...
template <class K, class V>
V& LPHashTable<K, V>::operator[](K const& key)//can we use this function to modify the value at that index?
{
