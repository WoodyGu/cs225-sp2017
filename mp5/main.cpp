...
int testTolerance(PNG imgIn, RGBAPixel target,int tolerance){
  int numLeaves = 0;
  for(size_t i = 0;i<imgIn.width();i++){
    for(size_t j = 0;j<imgIn.height();j++){
      int diff = pow((imgIn(i,j)->red-target.red),2)
                +pow((imgIn(i,j)->green-target.green),2)
                +pow((imgIn(i,j)->blue-target.blue),2);
      if(diff<tolerance){
        numLeaves++;
      }
    }
...
int main()
{
    ...
    */
    RGBAPixel test(92,73,96);
    cout<<"fullTree.numGoodsLeaves(test,1000) = "<<fullTree.numGoodsLeaves(test,666)<<endl;
    cout<<"the number of pixel satisfying this = "<<testTolerance(imgIn,test,666)<<endl;
