...
AnagramDict::AnagramDict(const string& filename)
{
    ...
    string word;
    //put all the words into the wordList
    ...
    for(string origWord : wordList){
      ...
      else{//if this pair already exists, then push the new anagram(this word) into the anagram vector list
...
{
    /* Your code goes here! */
    //do the samething excepe there is no fileIO
...
{
    /* Your code goes here! */
    //first we have to sort the word first to match the dictionary
    string sortChar = word;
    std::sort(sortChar.begin(),sortChar.end());
    auto lookup = dict.find(sortChar);
    if(lookup == dict.end()){
      //we did not find the word, bad input
      return vector<string>();
    }
    else{
      //we find the word and it have some anagram
      if(lookup->second.size()>1){
        return lookup->second;
      }
      else{//it does not have any anagram, so return Empty
        return vector<string>();
      }
    }
}
