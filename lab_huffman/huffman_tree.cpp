...
void HuffmanTree::buildTree(const vector<Frequency>& frequencies)
{
     ...
     */
     for(size_t i = 0;i<frequencies.size();i++){
       TreeNode* elem = new TreeNode(frequencies[i]);
       singleQueue.push(elem);
     }
     while((singleQueue.size()+mergeQueue.size())!=1){
      TreeNode* child1 = removeSmallest(singleQueue,mergeQueue);
      TreeNode* child2 = removeSmallest(singleQueue,mergeQueue);
      TreeNode* croot = new TreeNode(child1->freq.getFrequency()+child2->freq.getFrequency());
      croot->left = child1;
      croot->right = child2;
      mergeQueue.push(croot);
    }
...
void HuffmanTree::decode(stringstream& ss, BinaryFileReader& bfile)
{
    ...
    while (bfile.hasBits()) {
         ...
         */
        if(current->left == NULL&&current->right == NULL){//if it is the leaf, not current == NULL!
...
void HuffmanTree::writeTree(TreeNode* current, BinaryFileWriter& bfile)
{
     ...
     */
     ...
     if(current->left==NULL&&current->right==NULL){//if it is the leaf
       //write bit 1
       bfile.writeBit(1);
       bfile.writeByte(current->freq.getCharacter());//should put char in
     }
     else{
       //write bit 0
       bfile.writeBit(0);
       writeTree(current->left,bfile);
       writeTree(current->right,bfile);//recursive call
     }
}
...
HuffmanTree::TreeNode* HuffmanTree::readTree(BinaryFileReader& bfile)
{
     ...
     */
     if(!bfile.hasBits()){//check if the file has no more bits, don't need a while loop, base case 1
       return NULL;
     }
     TreeNode* node = NULL;
      if(bfile.getNextBit()==1){//a leaf node, base case 2
        Frequency f(bfile.getNextByte(),0);//construct a frequency object with char read from file and freq 0
        node = new TreeNode(f);//leaf node
      }
      else{
        node = new TreeNode(0);//new internal node with no char in it and freq 0
        node->left = readTree(bfile);//read tree for
