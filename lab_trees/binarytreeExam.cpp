...
template <typename T>
void BinaryTree<T>::insert(T &val, vector<int> location) {
...
template <typename T>
void BinaryTree<T>::transformToBinary()
{
    //your codes here!!!!
    if(root == NULL){
      return;
    ...
    root->right = NULL;
    transformToBinaryR(root->children[0],root->children,1);
}

template <typename T>
void BinaryTree<T>::transformToBinaryR(Node *node, vector<Node *>siblingList,size_t nextSiblingIndex){
  if(node == NULL||nextSiblingIndex>=siblingList.size()){//if the node have no children, return
    return;
  }
  else{
  ...
  //correct code!!!!
/*
  if(node->children.empty()&&nextSiblingIndex>=siblingList.size()){
    node->left = NULL;
    node->right = NULL;
    return;
  }
  if(!node->children.empty()&&nextSiblingIndex>=siblingList.size()){
    transformToBinaryR(node->children[0],node->children,1);
    node->left = node->children[0];
    node->right = NULL;
    return;
  }
  if(node->children.empty()&&nextSiblingIndex<siblingList.size()){
    transformToBinaryR(siblingList[nextSiblingIndex],siblingList,nextSiblingIndex+1);
    node->left = NULL;
    node->right = siblingList[nextSiblingIndex];
    return;
  }
...
/*
template <typename T>
void BinaryTree<T>::transformToBinaryS(Node *node)
{
  if(node == NULL){
    return;
  }
  ...
  for(size_t i = 1;i<node->children.size();i++){
    node->left->right = node->children[i];
  }
}
*/
///////////////////////////////////////
...
if(node == NULL){
  ...
  transformToBinaryS(node);
  for(size_t i = 0;i<node->children.size();i++){
    transformToBinaryR(node->children[i]);
  }
}
*/

template <typename T>
void BinaryTree<T>::preorderTraversal(Node* croot)
{
  if(croot!=NULL){
    cout<<croot->elem<<" ";
    preorderTraversal(croot->left);
    preorderTraversal(croot->right);
  }
}
