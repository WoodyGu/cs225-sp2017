...
*/
template <typename T>
typename BinaryTree<T>::Node* BinaryTree<T>::copy(const Node* subRoot)//root first and then children(preorder)
{
    if (subRoot == NULL)
        return NULL;
...
template <typename T>
void BinaryTree<T>::clear(BinaryTree::Node* subRoot)//children first and then root(post order)
{
    if (subRoot == NULL)
        return;
